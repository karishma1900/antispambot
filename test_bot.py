...
from bot import handle_new_chat_members  # Importing your main bot's functionality
...
async def start(message: types.Message):
    ...
    # Set up the state to wait for the group username
    await bot.set_state(message.from_user.id, "waiting_for_group_username")

# Handler for receiving group username input
@bot.message_handler(state="waiting_for_group_username")
async def receive_group_username(message: types.Message):
